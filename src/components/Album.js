...
import './album.css';
 ...
 class Album extends Component {
  ...
  formatTime(timeInSeconds){
    var min = Math.floor(timeInSeconds/60.00);
    var sec;
    sec = Math.round(timeInSeconds);
    if (sec < 60) {
     sec = Math.round(timeInSeconds);
   } else {
     sec = Math.floor(timeInSeconds) - (min * 60);
   }
    if (sec < 10) { sec = "0" + Math.round(sec);}
    var str = min + ":" + sec;
    return str;
  }
   ...
   return (
    <section className="album">
      ...
      <table id="song-list">
       ...
       <tr className={this.getSongClass(song)} key={index} onClick={()=> this.handleSongClick(song)}>
            <td>
            <button>
